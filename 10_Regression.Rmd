...
ggplot(data, aes(x=Fitted, y=Residual)) +
  geom_point() +
  facet_wrap(~Type) +
...
P1 <- 
  data %>% add_row(x=10.5, y=28, Type='Added') %>%
  ggplot(aes(x=x, y=y)) +
...
P2 <- 
  data %>% add_row(x=2, y=28, Type='Added') %>%
  ggplot(aes(x=x, y=y)) +
  geom_point(aes(shape=Type, color=Type)) +
  geom_smooth(method='lm', se=FALSE, color='red', linetype='dashed') +
  scale_color_manual(values=c('red','black')) +
...
data <- data.frame(x = `^`(seq(0.001,1.26,length=100), 10)) %>%
  mutate(`log(x)`=log(x),
         `Sqrt(x)`=sqrt(x),
         `1/x`= 1/x) %>%
  tidyr::pivot_longer(`log(x)`:`1/x`, 'Transformation') 

ggplot(data, aes(x=x, y=value, color=Transformation)) +
  geom_line() +
  coord_cartesian(xlim=c(0,10), ylim=c(-5,10)) +
  annotate('text', x=5.5, y=-1, color='red',
           label=latex2exp::TeX('$\\frac{1}{x}$')) +
  annotate('text', x=8, y=1.5, color='green',
           label=latex2exp::TeX('$\\log{(x)}$')) +
  annotate('text', x=8, y=3.5, color='blue',
           label=latex2exp::TeX('$\\sqrt{x}$')) +
  theme(legend.position = 'none')
```
