...
1. We will perform a regression analysis on the following data and use different bootstrap resampling methods to create a confidence interval for the slope parameter. In this case the residuals are symmetric, though perhaps we don't want to assume normality. 
...
```
    ...
    b) Using R, calculate the ratio $\hat{\rho}=\bar{y}_{T}/\bar{y}_{A}$. Hint: what is returned by coef(model)[1]?
